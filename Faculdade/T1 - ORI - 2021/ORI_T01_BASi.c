...
void remover_cliente(char *cpf) {
    ...
    int tam = 0;
    for(; tam < 4 && pix.chaves_pix[tam][0] != '\0'; tam = tam + 1)
    {
    ...
    {
        for(int j = i + 1; j < tam; j = j + 1)
        {
            if(strcmp(pix.chaves_pix[i], pix.chaves_pix[j]) > 0)
            {
                char aux[TAM_CPF];
                strcpy(aux, pix.chaves_pix[i]);
                strcpy(pix.chaves_pix[i], pix.chaves_pix[j]);
                strcpy(pix.chaves_pix[j], aux);
            }
        }
    }
    int i = 0;
    chaves_pix_index p;
    strcpy(p.chave_pix, pix.chaves_pix[0]);
    chaves_pix_index *pi = bsearch(&p, chaves_pix_idx, qtd_chaves_pix, sizeof(chaves_pix_index), qsort_chaves_pix_index);
    ...
    /*int indice = 0;
    for(int i = 0; i < qtd_chaves_pix; i = i + 1)
    {
        if(strcmp(chaves_pix_idx[i].chave_pix, pix.chaves_pix[indice][1]) == 0)
        {
            indice = indice + 1;
        }
    }
...
void alterar_saldo(char *cpf, double valor) {
    ...
    se nao achar algo (==NULL) então não prossego, pois nao esta cadastrado*/
    if(valor == 0)
    {
